...
class InitPelunasanKreditTableSeeder extends Seeder
{
	public function run()
	{
		//1. simpan imigrasi
		...
		$pengajuan 		= Pengajuan::where('akses_koperasi_id', TAuth::activeOffice()['koperasi']['id'])->where('status', 'realisasi')->take(Pengajuan::where('akses_koperasi_id', TAuth::activeOffice()['koperasi']['id'])->where('status', 'realisasi')->count()/2)->get();
