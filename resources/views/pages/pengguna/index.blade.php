		...
		<div class="{{ (isset($page_datas->id) ? 'hidden-xs' : '') }} col-sm-3 content-sidebar">
			...
			<div class="sidebar-content _window" data-padd-top="auto" data-padd-bottom="39">
				<div class="list-group">
				    @foreach($page_datas->pengguna as $key => $value)
				        <a href="{{ route('pengguna.show', array_merge(['id' => $value['id']], Input::all())) }}" class="list-group-item {{ $key == 0? 'first': '' }} {{ ((isset($page_datas->id) && $page_datas->id == $value['id']) ? 'active' : '') }}">
