...
<ul class="pagination">
	<li class="page-item {{ $paging  == 1 ? 'disabled' : '' }}">
		<a class="page-link no-decoration {{ $paging  == 1 ? 'disabled' : '' }}" href="{{ $paging  == 1 ? '#' : route('credit.index', ['page' => ($paging -1)]) }}""><i class="fa fa-angle-left"></i></a>
	</li>
	...
	@if ($total_page <= ((int)$paging  + floor($range/2)))
		...
		@for ($i = 1; $i <= min($range, $total_page-$start); $i++)
			<li class="page-item {{ $paging  == $start+$i ? 'active' : '' }} hidden-sm-up">
				<a href="{{ $page_attributes->paging->url($start+$i) }}" class='page-link no-decoration'>
					{{ $start + $i }}
				</a>
			</li>
		@endfor					
		...
		@for ($i = 0; $i < $range; $i++)
			@if (max(1, $paging  - 2) + $i <= $total_page)
				<li class="page-item {{ $paging  == max(1, $paging  - 2) + $i? 'active' : '' }} hidden-sm-up">
					<a href="{{ $page_attributes->paging->url(max(1, $paging  - 2) + $i) }}" class='page-link no-decoration'>
						{{ max(1, $paging  - 2) + $i }}
					</a>
				</li>
			@endif
	...
	<li class="page-item {{ $paging  == $total_page ? 'disabled' : '' }}">
		<a class="page-link no-decoration" href="{{ $paging  == $total_page ? '#' : route('credit.index', ['page' => ($paging +1)]) }}""><i class="fa fa-angle-right"></i></a>
	</li>						
</ul>
